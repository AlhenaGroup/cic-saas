          ...
          <tbody>
            {filtered.map(cl => {
              ...
              return <tr key={cl.id} style={{ borderBottom: '1px solid #1a1f2e', cursor: 'pointer' }} onClick={() => setEditing(cl)}>
                ...
                <td style={{ ...S.td, color: 'var(--text2)', fontSize: 11 }}>{cl.google_sheet_tab || '—'}</td>
                <td style={S.td}>
                  {cl.attivo
                ...
                <td style={S.td} onClick={e => e.stopPropagation()}>
                  ...
                  <button onClick={() => duplicate(cl)} style={{ background: 'none', border: '1px solid var(--border)', color: 'var(--text2)', padding: '3px 8px', borderRadius: 4, fontSize: 11, cursor: 'pointer', marginRight: 4 }}>Duplica</button>
...
function ResponsesViewer({ checklist, onClose }) {
  ...
  return <div className="m-modal-fullscreen" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 200, padding: 24, overflow: 'auto' }}>
    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 880 }}>
      <div style={{ padding: 16, borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h3 style={{ margin: 0, fontSize: 15 }}>Risposte: {checklist.nome}</h3>
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{checklist.locale} · {checklist.reparto} · {checklist.momento}</div>
        </div>
                      ...
                      <tbody>
                        {items.map(it => {
                          ...
                          return <tr key={it.id} style={{ borderBottom: '1px solid #1a1f2e' }}>
                            <td style={{ ...S.td, color: 'var(--text2)', verticalAlign: 'top', maxWidth: 380 }}>{it.label}</td>
                        ...
                        {Object.keys(ans).filter(k => !itemById[k]).map(k => (
                          <tr key={k} style={{ borderBottom: '1px solid #1a1f2e', opacity: .6 }}>
                            <td style={{ ...S.td, color: 'var(--text3)', fontStyle: 'italic' }}>(item rimosso) {k.substring(0, 8)}</td>
    ...
    <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 720 }}>
      ...
      <div style={{ padding: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
          ...
          <label>
            <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 4 }}>Locale</div>
          ...
          <label>
            <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 4 }}>Reparto</div>
            <select value={c.reparto || ''} onChange={e => updateField('reparto', e.target.value)} style={{ ...iS, width: '100%' }}>
              {REPARTI.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
          </label>
          <label>
            <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 4 }}>Momento</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10, marginBottom: 14 }}>
          <label>
            <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 4 }}>Tab Google Sheet (opzionale)</div>
            ...
            <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2 }}>Le risposte vengono scritte sullo sheet del locale, in questo tab.</div>
          </label>
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14, marginTop: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>Domande ({(c.items || []).length})</div>
            ...
            return <div key={it.id}
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start' }}>
                <div title="Trascina per riordinare"
                  style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'grab', color: 'var(--text3)', fontSize: 14, lineHeight: 1, padding: '0 4px', userSelect: 'none' }}>
                  ⋮⋮
                </div>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--text2)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
          ...
          <button onClick={save} disabled={saving}
            style={{ ...iS, background: '#10B981', color: 'var(--text)', fontWeight: 700, border: 'none', padding: '8px 20px', cursor: saving ? 'wait' : 'pointer' }}>
            {saving ? 'Salvo…' : 'Salva'}
          </button>
        </div>
      </div>
