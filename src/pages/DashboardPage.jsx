...
export default function DashboardPage({ settings }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const spFilter = selectedSp === 'all'
        ? salesPoints.map(s => s.id)
        : [parseInt(selectedSp)]
      const d = await getReportData(token, { from, to, idsSalesPoint: spFilter })
      setData(d)
    ...
    <div style={{background:'var(--surface)',borderBottom:'1px solid var(--border)',padding:'0 1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between',height:'56px',position:'sticky',top:0,zIndex:10}}>
      <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
        ...
        {salesPoints.length>0&&<select value={selectedSp} onChange={e=>setSelectedSp(e.target.value)} style={iStyle}>
          ...
          {salesPoints.map(sp=><option key={sp.id} value={sp.id}>{sp.description||sp.name||'Locale '+sp.id}</option>)}
        </select>}
      </div>
    ...
    <div style={{padding:'1.5rem',maxWidth:'1200px',margin:'0 auto'}}>
      ...
      {loading?<Loader/>:<>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'12px',marginBottom:'1.25rem'}}>
          ...
          <KpiCard label="Scontrini"       value={fmtN(data?.scontrini)} sub="nel periodo"     color="var(--text)"/>
          <KpiCard label="Scontrino medio" value={fmt(data?.medio)}      sub="per documento"   color="var(--green)"/>
          <KpiCard label="Reparti attivi"  value={depts.filter(d=>d.profit>0).length} sub="con vendite" color="var(--text)"/>
        </div>

        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'1.25rem'}}>
          <Section title="Venduto per reparto">
            {depts.length===0
              ?<div style={{fontSize:'13px',color:'var(--text3)',textAlign:'center',padding:'1rem'}}>Nessun dato</div>
              :depts.slice(0,8).map((d,i)=><BarRow key={i} label={d.department?.description||'Reparto'} value={d.profit} total={totale} color={i===0?'var(--blue)':i===1?'#6B7FCC':'#9BA8D9'}/>)}
          </Section>
          <Section title="Venduto per categoria">
            {cats.length===0
              ?<div style={{fontSize:'13px',color:'var(--text3)',textAlign:'center',padding:'1rem'}}>Nessun dato</div>
              :cats.slice(0,8).map((c,i)=><BarRow key={i} label={c.category?.description||'Categoria'} value={c.totalSold} total={cats.reduce((s,x)=>s+(x.totalSold||0),0)} color={i===0?'#16A34A':i===1?'#4ADE80':'#86EFAC'}/>)}
          </Section>
        </div>
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'13px'}}>
              <thead><tr style={{borderBottom:'1px solid var(--border)'}}>
                {['Aliquota','Imponibile','IVA','Totale','% sul totale'].map(h=><th key={h} style={{padding:'6px 12px 10px',textAlign:'left',fontWeight:'500',fontSize:'11px',color:'var(--text3)',textTransform:'uppercase',letterSpacing:'.04em'}}>{h}</th>)}
              </tr></thead>
              <tbody>
                {taxes.length===0
                  ?<tr><td colSpan={5} style={{padding:'1rem',textAlign:'center',color:'var(--text3)'}}>Nessun dato</td></tr>
                  :taxes.map((t,i)=>{
                    ...
                    return <tr key={i} style={{borderBottom:'1px solid var(--border)'}} onMouseEnter={e=>e.currentTarget.style.background='var(--surface2)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      ...
                      <td style={{padding:'10px 12px',color:'var(--text2)'}}>{fmt(t.tax_amount)}</td>
