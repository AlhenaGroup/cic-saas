...
// Tutte le chiamate a CiC passano per il proxy Vercel (server-to-server)
// Stesso approccio di IAMMI con Google Cloud Functions
const PROXY = '/api/cic';
...
export async function getToken(apiKey) {
  ...
  await proxyCall(apiKey, 'token');
  return apiKey; // restituiamo la key stessa come "token" — il proxy la usa internamente
}
    ...
    if (Array.isArray(d.receipts) && d.receipts.length > 0) {
      ...
      receipts.forEach(r => {
        ...
        (r.items || []).forEach(item => {
          ...
          deptMap[dept].profit += price;
          deptMap[dept].qty    += item.quantity || 1;
      ...
      return {
        totale, scontrini: receipts.length,
        ...
        trend: demo.trend, topProducts: demo.topProducts,
        scontriniList: demo.scontriniList, prodOre: demo.prodOre,
        suspicious: demo.suspicious, fatture: demo.fatture, ce: demo.ce,
        isDemo: false
      };
    }
  } catch(e) {
    console.warn('[CIC] receipts non disponibili:', e.message, '— uso demo');
  }
...
// ── Dati demo realistici (usati finché CiC non abilita receipts) ──────────
...
export function generateDemoData(from, to, salesPoints = []) {
  ...
  while (cur <= end) {
    ...
    const v = Math.max(0, Math.round((isWe ? 2400 : 1500) + (Math.random() - 0.5) * 600));
    trend.push({ date: cur.toISOString().split('T')[0], label: cur.toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit' }), ricavi: v, scontrini: Math.round(v / 22) });
    cur.setDate(cur.getDate() + 1);
  }

  const payments = ['Contanti','Carta','Satispay','Carta','Contanti','Carta'];
  const scontriniList = Array.from({ length: Math.min(50, Math.round(100*m)) }, (_, i) => {
    const d = new Date(from); d.setDate(d.getDate() + rand(0, days-1));
    return { id: 'S' + String(i+1).padStart(4,'0'), date: d.toISOString().split('T')[0], time: String(rand(11,23)).padStart(2,'0')+':'+String(rand(0,59)).padStart(2,'0'), items: rand(1,8), total: Math.round((8 + Math.random()*87)*100)/100, payment: payments[rand(0,5)], locale: salesPoints[rand(0, Math.max(0,salesPoints.length-1))]?.description || 'REMEMBEER' };
  }).sort((a,b) => b.date.localeCompare(a.date) || b.time.localeCompare(a.time));

  const prodOre = Array.from({ length: 16 }, (_, i) => {
    const h = i + 8, peak = (h >= 12 && h <= 14) || (h >= 19 && h <= 22);
    const v = Math.max(0, Math.round((peak ? 2800 : 400) * (0.7 + Math.random() * 0.6)));
    return { ora: String(h).padStart(2,'0') + ':00', ricavi: v, scontrini: Math.round(v / 22) };
  });

  const suppliers = ['DAVIDE MEINI','LA NOBILE BEVERAGE','DISTRIBUZIONE METRO','FORNITORE VINI SRL','ALHENA SERVIZI'];
  const fatture = Array.from({ length: Math.min(30, Math.round(40*m)) }, (_, i) => {
    const d = new Date(from); d.setDate(d.getDate() + rand(0, days-1));
    return { id: i+1, date: d.toISOString().split('T')[0], fornitore: suppliers[rand(0,4)], numero: 'FT'+rand(100,999), tipo: 'TD01', locale: salesPoints[rand(0,Math.max(0,salesPoints.length-1))]?.description||'REMEMBEER', imponibile: Math.round((120+Math.random()*3080)*100)/100, iva: Math.round((12+Math.random()*628)*100)/100, statoSDI: Math.random()>.3?'✅ Consegnata':'⏳ In attesa', statoContabile: Math.random()>.4?'✅ Registrata':'📋 Da registrare' };
  ...
  const foodCost = Math.round(totale*0.195), bevCost = Math.round(totale*0.148);
  const matCost = Math.round(totale*0.018), strCost = Math.round(totale*0.025);
  const totCosti = foodCost + bevCost + matCost + strCost;
  ...
  return {
    totale, scontrini, medio: scontrini > 0 ? totale / scontrini : 0,
    depts, cats, taxes, trend, topProducts, scontriniList, prodOre, suspicious, fatture,
    ce: { ricavi: totale, foodCost, bevCost, matCost, persCost: 0, strCost, altCost: 0, totCosti, mol, molPct: totale > 0 ? mol/totale*100 : 0, foodPct: totale > 0 ? foodCost/totale*100 : 0, bevPct: totale > 0 ? bevCost/totale*100 : 0, persPct: 0 },
    isDemo: true
  };
}
