...
export async function getSoldByDepartment(token, { from, to, idsSalesPoint }) {
  ...
  const data = await cicGet(token, '/reports/sold-by-department', {
    filter: JSON.stringify(f), start: 0, limit: 100
  })
...
export async function getSoldByCategory(token, { from, to, idsSalesPoint }) {
  ...
  if (idsSalesPoint?.length) f.idsSalesPoint = idsSalesPoint
  const data = await cicGet(token, '/reports/sold-by-category', {
    filter: JSON.stringify(f), start: 0, limit: 100
  })
  return Array.isArray(data.records) ? data.records : []
}
...
export async function getSoldByTax(token, { from, to, idsSalesPoint }) {
  ...
  if (idsSalesPoint?.length) f.idsSalesPoint = idsSalesPoint
  const data = await cicGet(token, '/reports/sold-by-tax', {
    filter: JSON.stringify(f), start: 0, limit: 50
  })
  return Array.isArray(data.records) ? data.records : []
}
...
export async function getReceipts(token, { from, to, idsSalesPoint, start = 0, limit = 50 }) {
  ...
  const data = await cicGet(token, '/documents/receipts', p)
  return { records: data.receipts || [], total: data.totalCount || 0 }
}
