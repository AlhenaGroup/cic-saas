...
export function costOfManualArticle(art, articlesPriceByName, manualByName, depth = 0, placeholdersByName = null) {
  ...
  for (const ingr of (art.ingredienti || [])) {
    ...
    // Scarto: 3 modalita' supportate.
    //  1) scarto in UM (stessa baseUm di qty): qty_lorda = qty + scarto
    //     es. cipolla 200g netti + 50g scarto pelle = 250g pagati
    //  2) scarto in percentuale (scarto_unita === '%'): qty_lorda = qty / (1 - pct/100)
    //     es. cipolla 200g netti, scarto 20% = 250g pagati
    //  3) retrocompat scarto_pct (campo legacy): stessa formula del 2
    let qtyLorda = base.qty
    if (ingr.scarto != null && Number(ingr.scarto) > 0) {
      const scU = (ingr.scarto_unita || '').toString().trim()
      if (scU === '%') {
        const pct = Math.max(0, Math.min(99, Number(ingr.scarto)))
        qtyLorda = base.qty / (1 - pct / 100)
      } else {
        const sc = toBaseUnit(Number(ingr.scarto), scU || ingr.unita)
