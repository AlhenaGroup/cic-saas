...
export default async function handler(req, res) {
  ...
  const { file_path, doc_id, doc_type, action, pdfBase64 } = req.body || {};

  // ─── Action: parse-invoice-pdf — estrai testo raw da PDF base64 ───────────
  if (action === 'parse-invoice-pdf') {
    ...
    try {
      ...
      console.error('[PARSE-INVOICE-PDF]', err.message);
